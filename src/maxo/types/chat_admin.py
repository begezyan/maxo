...
class ChatAdmin(MaxoType):
    ...
    Args:
        ...
        permissions: Перечень прав доступа пользователя. Возможные значения:
            - `"read_all_messages"` — Читать все сообщения. Это право важно при назначении ботов: без него бот не будет получать обновления через [Webhook](https://dev.max.ru/docs-api/methods/POST/subscriptions) в групповом чате или канале
