...
from maxo.dialogs.widgets.common import ManagedScroll, sync_scroll
...
from maxo.dialogs.widgets.text import Const, Format, List, ScrollingText
...
async def product_getter(**__: Any) -> dict[str, Any]:
    return {
        "products": [(f"Product {i}", i) for i in range(1, 30)],
    }


async def paging_getter(dialog_manager: DialogManager, **__: Any) -> dict[str, Any]:
    stub_scroll = cast(ManagedScroll, dialog_manager.find(ID_STUB_SCROLL))
    current_page = await stub_scroll.get_page()
    return {
        "pages": 7,
